...
public class AbstractMapper {
    ...
    /**
     * filling from $ref Objects, inner schemas, maps, etc.
     */
    public void fillVariableProperties(String schemaName,
        ...
        // ✅ 1. Если propertiesMap пустой (например, comment: {}), интерпретируем как Object
        ...
        // ✅ 2. Важно: если type=object, но указан format → это НЕ общий объект, а скалярный тип (uuid/date/etc)
        // Поэтому применяем format ДО fallback-логики
        if (OBJECT_TYPE.equalsIgnoreCase(type) && format != null &&
            JAVA_LOWER_CASE_TYPES_CHECK_CONVERTER.containsKey(format)) {
            // Применяем format и сразу выходим — setFormat уже проставил корректный .type
            variableProperties.setFormat(format);
            return;
        }

        // fallback: устанавливаем тип (Object если null или object)
        ...
        // Полиморфизм (oneOf/allOf/anyOf)
            ...
            // Слияние properties из всех полиморфных схем
            Map<String, Object> mergedProperties = polymorphList.stream()
                    .flatMap(ref -> {
                        String refStr = ref.toString();
                        // Извлекаем имя схемы из $ref (поддержка Map и String)
        ...
        // Fallback: type = object, но нет properties/enum/$ref → Object
        else {
            if (OBJECT_TYPE.equals(variableProperties.getType())) {
                variableProperties.setType(OBJECT_TYPE);
                variableProperties.setValid(false);
            }
            // ✅ Убедимся, что format применён даже для scalar-типов (например, type: string, format: uuid)
            variableProperties.setFormat(format);
        }

        // ✅ Добавляем импорты для коллекций/мапов, если нужно
    ...
    // ✅ ВСЕГДА используем commonPackage как базовый
    ...
    // ✅ Поддержка external package из $ref
    private void fillReferenceProperties(String schemaName,
        ...
        if (variableProperties.getType() == null || OBJECT_TYPE.equals(variableProperties.getType())) {
            ...
            if (getStringValueIfExistOrElseNull(refReplace, schemas) != null
                ...
                // Поддержка external package (внедрённого через getContent)
    ...
    private void fillArrayProperties(String schemaName,
            ...
            if (getStringValueIfExistOrElseNull(FORMAT, items) != null || getStringValueIfExistOrElseNull(TYPE, items) != null) {
                if (getStringValueIfExistOrElseNull(FORMAT, items) != null) {
                    if (getStringValueIfExistOrElseNull(FORMAT, items).equals(EXISTING)) {
                        ...
                        // Fill java format attributes array
        ...
        if (variableProperties.getItems() != null &&
            ...
            // Поддержка external package в items
